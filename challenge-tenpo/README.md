...
- **UI (destinatarios):** La sección de destinatarios (`RecipientSelector` + `RecipientForm`) permite crear y seleccionar destinatarios Tenpo; el hook `useRecipients` consume `/recipient` y mantiene la lista sincronizada vía React Query.
