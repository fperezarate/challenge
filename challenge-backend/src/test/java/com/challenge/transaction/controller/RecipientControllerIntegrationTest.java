	...
	class Create {
		...
		void createsValidRecipientAndPersists() throws Exception {
			...
			mockMvc.perform(
					...
					.andExpect(jsonPath("$.nombre", is("Nombre Destinatario")))
					.andExpect(jsonPath("$.rut", is("11.111.111-1")))
					.andExpect(jsonPath("$.numeroCuenta", is("123456789")))
					.andExpect(jsonPath("$.email", is("correo@ejemplo.cl")))
					.andExpect(jsonPath("$.tipoCuenta", is("Tenpo")));
		}
